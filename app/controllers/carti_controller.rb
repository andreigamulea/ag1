class CartiController < ApplicationController
  def index
    @products = Product.select(:id, :name, :price, :stock, :stock_status, :custom_attributes)
                      .joins(:categories)
  ...
  def show
    @product = Product.select(:id, :name, :price, :description, :stock, :stock_status, :custom_attributes)
                     .includes(:main_image_attachment, :secondary_images_attachments)
                     .find(params[:id])
