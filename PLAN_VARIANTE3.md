# PLAN VARIANTE V3.7 - Relaxed Enterprise
...
class M2CleanupVariantsData < ActiveRecord::Migration[7.0]
  def up
    ...
    execute(<<~SQL)
      DELETE FROM product_option_types
      WHERE id NOT IN (
        SELECT MIN(id) FROM product_option_types GROUP BY product_id, option_type_id
      )
    SQL
    execute(<<~SQL)
      DELETE FROM option_value_variants
      WHERE id NOT IN (
        SELECT MIN(id) FROM option_value_variants GROUP BY variant_id, option_value_id
      )
    SQL
...
class Variant < ApplicationRecord
  ...
  # Validare conflict la activare (transformă 500 în mesaj user-friendly)
...
module Variants
  class CreateOrReactivateService
    ...
    def call(option_value_ids, attributes = {})
      ...
      desired_status = (attributes[:status] || attributes["status"]).presence || :active
      ...
      if digest.nil? && desired_status.to_sym == :inactive
    ...
    def handle_unique_violation(exception, digest)
      ...
      # Race condition pe digest - încearcă să găsească varianta existentă
    ...
    def valid_option_values?(ids)
      ...
      # Verifică că type-urile aparțin produsului
    ...
    def create_new(option_value_ids, attributes)
      # Păstrează statusul dorit (default: active) - robust pentru string keys + nil
      desired_status = (attributes[:status] || attributes["status"]).presence || :active
...
| Acțiune | WooCommerce | Shopify | Solidus | V3.7 |
|---------|-------------|---------|---------|------|
| Creare variantă | ✅ | ✅ | ✅ | ✅ |
| Dezactivare | ✅ | ✅ | ✅ | ✅ |
| Reactivare | ✅ | ✅ | ✅ | ✅ |
